...
# --- 8. รับ Notify จาก Streamlit (ปรับปรุงเพื่อส่งให้ทุกคนที่เป็นเพื่อนกับบอท) ---
...
async def notify_booking(request: Request):
    ...
    # 1. กรณีส่งตารางสรุป (ส่งให้ทุกคนที่เป็นเพื่อนกับ BOT)
    if mode == "all_schedule":
        try:
            now = datetime.now().isoformat()
            # ดึงข้อมูลที่อนุมัติแล้วทั้งหมดมาทำตาราง
            res = supabase.table("bookings").select("*").eq("status", "Approved").gt("end_time", now).order("start_time").execute()
            
            # ใช้คำสั่ง broadcast เพื่อส่งหาเพื่อนทุกคน
            line_bot_api.broadcast([
                TextSendMessage(text="📢 นี่คือตารางงานล่าสุด"),
                create_schedule_flex("📅 ตารางการใช้งานปัจจุบัน", res.data, "#2E7D32")
            ])
        except Exception as e:
            print(f"Broadcast Schedule Error: {e}")

    # 2. กรณีแจ้งเตือนคนจองรายบุคคล (ถ้ามี User ID ของคนจอง)
    # หมายเหตุ: ถ้าต้องการแจ้งเฉพาะคนจอง ต้องเก็บ line_user_id ไว้ในฐานข้อมูลตอนจองด้วย
    elif mode == "private_noti":
        user_id = data.get("line_user_id")
        if user_id:
            msg = f"🔔 แจ้งผลการจองของคุณ: {data.get('status')}\nรายการ: {data.get('resource')}"
            line_bot_api.push_message(user_id, TextSendMessage(text=msg))

    # 3. กรณีมีคนจองเข้ามาใหม่ (แจ้งให้ Admin/ทุกคน ทราบเพื่อรออนุมัติ)
    else:
        booking_id = data.get("id")
        # แจ้งเตือนแบบ broadcast เพื่อให้ Admin เห็นปุ่มอนุมัติ
